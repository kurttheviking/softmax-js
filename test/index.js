...
describe('instance', function () {
  ...
  it ('throws TypeError when passed gamma below 0', function () {
    function init () {
      new Bandit({gamma: -1});
    }
  ...
  it ('throws TypeError when passed tau below 0', function () {
    function init () {
      new Bandit({tau: -1});
    }
  ...
  describe('.serialize', function () {
    ...
    it('resolves to snapshot of algorithm state (with gamma)', function () {
      var arms = 5;
      var gamma = 1e-9;
      var interations = 100;
      ...
      .then(function (pickle) {
        ...
        chai.expect(pickle.gamma).to.equal(gamma);
        chai.expect(pickle.tau).to.equal(null);
        chai.expect(pickle.counts.length).to.equal(arms);
        chai.expect(pickle.values.length).to.equal(arms);
      });
    });

    it('resolves to snapshot of algorithm state (with tau)', function () {
      var arms = 5;
      var tau = _.random(0, 1, true);
      var interations = 100;
      ...
      .then(function (pickle) {
        chai.expect(pickle.arms).to.equal(arms);
        chai.expect(pickle.tau).to.equal(tau);
        chai.expect(pickle.counts.length).to.equal(arms);
        chai.expect(pickle.values.length).to.equal(arms);
      });
    });
  ...
  describe('.load', function () {
    it('resolves to restored values (gamma)', function () {
      var arms = 5;
      var gamma = 1e-9;
      var interations = 100;
    ...
    it('restores previous state (gamma)', function () {
      var arms = 5;
      var gamma = 1e-9;
      var interations = 100;
    ...
    it('resolves to restored values (tau)', function () {
      var arms = 5;
      var tau = _.random(0, 1, true);
      var interations = 100;
    ...
    //      partial update can be restored and built into a state
    //      equivilent to a known full update
    it('restores previous state (tau)', function () {
      var arms = 5;
      var tau = _.random(0, 1, true);
      var interations = 100;
